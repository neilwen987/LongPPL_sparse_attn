...
def _init_rope_new(self):
    if self.config.rope_scaling is None:
        self.rotary_emb = modeling_qwen3.Qwen3RotaryEmbedding(
        ...
        if scaling_type == "linear":
            self.rotary_emb = modeling_qwen3.Qwen3LinearScalingRotaryEmbedding(
                self.head_dim,
                max_position_embeddings=self.max_position_embeddings,
                scaling_factor=scaling_factor,
            )
        elif scaling_type == "dynamic":
            self.rotary_emb = modeling_qwen3.Qwen3DynamicNTKScalingRotaryEmbedding(
                self.head_dim,
                max_position_embeddings=self.max_position_embeddings,
                scaling_factor=scaling_factor,
            )
...
def _rope_scaling_validation_new(self):
    ...
    if rope_scaling_type is None or rope_scaling_type not in [
        ...
        raise ValueError(
            f"`rope_scaling`'s name field must be one of ['linear', 'dynamic', 'yarn', 'longrope', 'llama3', 'eabf'], got {rope_scaling_type}"
        )
    if (
...
modeling_qwen3.apply_rotary_pos_emb = apply_rotary_pos_emb_scale
modeling_qwen3.Qwen3Attention._init_rope = _init_rope_new
configuration_qwen3.Qwen3Config._rope_scaling_validation = _rope_scaling_validation_new 